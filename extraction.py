...
def _rule_based_fallback(scrubbed: str) -> ExtractionResult:
    ...
    for variant, canonical in (("amlodipin", "Amlodipine"), ("amlodipine", "Amlodipine"), ("lisinopril", "Lisinopril")):
    ...
    if re.search(r"\b(?:hipertensi|hypertension|htn)\b", lowered):
        entities.append({"entity_type": "condition", "name": "Hypertension", "status": "active", "negated": False, "discontinued": False, "explicit_state_change": False, "raw_text_span": "hipertensi"})
    return ExtractionResult(
        entities=entities,
        warnings=["LLM/cache unavailable; narrow rule-based extraction used. Manual review required."],
        source="rule_based",
